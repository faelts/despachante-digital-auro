  ...
  return (
    <div className="fixed top-0 left-0 right-0 z-50 w-full font-sans">
      <div className={`bg-[#E11D48] text-white py-1.5 transition-all ${isScrolled ? 'h-0 py-0 overflow-hidden opacity-0' : 'h-auto'}`}>
        <div className="container mx-auto px-6 md:px-12 flex justify-between items-center text-[10px] md:text-[11px] font-bold uppercase tracking-wider">
      ...
      <header className={`transition-all ${isScrolled ? 'bg-[#0F172A]/95 py-2 shadow-2xl' : 'bg-[#0F172A] py-6'}`}>
        <div className="container mx-auto px-6 md:px-12 flex items-center justify-between">
          <div className="flex items-center h-10 md:h-14">
            <img src="logotipo-edit-branca-comprimida-maur-despachante.png" alt="Logo" className="h-full w-auto object-contain" />
          </div>
          <nav className="hidden lg:flex items-center gap-10 text-[11px] font-black uppercase text-white">
            ...
            <Button className="bg-[#E11D48] hover:bg-[#BE123C] text-white px-8 py-6 rounded-none shadow-xl" onClick={() => window.open(whatsappUrl, '_blank')}>
              Falar com Especialista
            </Button>
          </nav>
          <button className="lg:hidden p-2 text-white" onClick={() => setIsMenuOpen(!isMenuOpen)}>{isMenuOpen ? <X /> : <Menu />}</button>
        </div>
      </header>
    </div>
  );
